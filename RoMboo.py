...
def gEnNaMe():
    ...
    return 'S1X' + ''.join(random.choice(cHaRs) for _ in range(6)).upper()

def gEnPaSsWoRd(lEn=9):
    cHaRs = string.ascii_letters + string.digits
    rAnD = ''.join(random.choice(cHaRs) for _ in range(lEn)).upper()
    return f"15x3{rAnD}-RoMboo-15x3"
...
def cReAtEaCc(rEg):
    ...
    try:
        ...
        return gEtToKeN(uId, pAsS, rEg)
    except:
        return cReAtEaCc(rEg)

def gEtToKeN(uId, pAsS, rEg):
    ...
    return rEgMaJoR(aCcEsStOkEn, oPeNiD, fIeLd, uId, pAsS, rEg)
...
def rEgMaJoR(aT, oId, f, uId, pAsS, rEg):
...
def gEtLoGiNdAtA(jWt, pL, rEg):
    ...
    while aTt < mAxTrY:
        try:
            ...
            print(f"{R}{B}Request Fail: {e}. Try {aTt + 1}/{mAxTrY}")
            aTt += 1
            time.sleep(2)
    print(f"{R}{B}Max tries reached")
    return None
...
if __name__ == "__main__":
    ...
    rEg = input(f"{B}{R}[{L}+{R}]{R} Region (ME,IND,VN,BR...): {Fore.RESET}").upper()
    ...
    for i in range(70):
        with open('accounts.txt', 'a') as f:
            try:
                ...
                if cOdE == 200:
                    f.write(f"{uId}:{pAsS}\n")
                    print(f"{L}{B}{uId}|{pAsS} : {cOdE}")
                else:
                    print(f'{R}{B}Failed: {nAmE} | {cOdE}')
            except Exception as e:
                print(R + B + "Error: " + str(e))
